...
$sqlEntregas .= " GROUP BY p.ID, e.ID";
...
if ($resultEntregas) {
    // Inicialize um array para armazenar os dados da tabela de entregas
    $dataEntregasTable = array();

    // Adicione os cabeçalhos do array
    $dataEntregasTable[] = ['ID', 'Entrega', 'Percentual'];

    // Adicione os dados ao array
    while ($rowEntregas = mysqli_fetch_assoc($resultEntregas)) {
        $dataEntregasTable[] = [$rowEntregas['ID'], $rowEntregas['Titulo'], $rowEntregas['Percentual']];
    }
    ...
    $countEntregasSQL .= " GROUP BY p.ID";

    $resultCountEntregas = mysqli_query($conn, $countEntregasSQL);

    if ($resultCountEntregas) {
        while ($rowCountEntregas = mysqli_fetch_assoc($resultCountEntregas)) {
            $dataEntregas[] = [$rowCountEntregas['Projeto_Nome'], (int) $rowCountEntregas['total_por_entrega']];
        }
    } else {
    ...
    // Trate erros na consulta
    echo "Erro na consulta SQL para detalhes de entregas: " . mysqli_error($conn);
}
?>
...
<head>
    ...
    <script>
        ...
        function drawDashboard() {
            ...
            var dataEntregasTable = google.visualization.arrayToDataTable(<?php echo json_encode($dataEntregasTable); ?>);
            ...
            var tableEntregas = new google.visualization.ChartWrapper({
                'chartType': 'Table',
                'containerId': 'tableEntregas',
                'options': {
                    'width': '50%',
                    'height': '300px'
                }
            ...
            dashboard.bind(projetoSelector, [chartEntregas, tableEntregas]);
    ...
    <div class="container main">
        ...
        <div class="main mx-auto tabela-projetos">
            ...
            <div id="dashboard">
                ...
                <!-- Tabela de entregas -->
                <div id="tableEntregas" style="float: left;"></div>
            </div>
